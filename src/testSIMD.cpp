//
// Created by arbeit on 8/20/25.
//
// compile with: g++ -O3 -march=native -fopenmp -std=c++17 cross_bench_no_lambdas.cpp -o cross_bench_no_lambdas
#include <iostream>
#include <chrono>
#include <random>
#include <cassert>
...
void crossProduct3x3_tensor(const Tensor3f& A,
    ...
    Tensor<float, 2> Cx = (Ay * Bz) - (Az * By);
    Tensor<float, 2> Cy = (Az * Bx) - (Ax * Bz);
    Tensor<float, 2> Cz = (Ax * By) - (Ay * Bx);
...
void crossProduct3x3_standard_loop(Tensor3f& A, Tensor3f& B, Tensor3f& C) {
    ...
    for (long i = 0; i < rows; ++i){
        for (long j = 0; j < cols; ++j){
            C(i, j, 0) = A(i, j, 2) * B(i, j, 1) - A(i, j, 1) * B(i, j, 2);
            C(i, j, 1) = A(i, j, 0) * B(i, j, 2) - A(i, j, 2) * B(i, j, 0);
            C(i, j, 2) = A(i, j, 1) * B(i, j, 0) - A(i, j, 0) * B(i, j, 1);
        }
    }
}
...
void crossProduct3x3_parallel(Tensor2f& A, Tensor2f& B, Tensor2f& C) {
    const auto& dims = A.dimensions();
    long rows_cols = dims[0]; // count
#pragma omp parallel for
    for (long i = 0; i < rows_cols; ++i){
        // consistent formula with others:
        C(i, 0) = A(i, 2) * B(i, 1) - A(i, 1) * B(i, 2);
        C(i, 1) = A(i, 0) * B(i, 2) - A(i, 2) * B(i, 0);
        C(i, 2) = A(i, 1) * B(i, 0) - A(i, 0) * B(i, 1);
    }
}
...
void crossProduct3x3_SIMD_from_tensor(const Tensor3f& A,
    ...
    // Allocate aligned SoA buffers (32-byte)
    ...
    // SIMD cross product (assumes AVX2; aligned)
    size_t i = 0;
    for (; i + 7 < count; i += 8) {
        __m256 a0 = _mm256_load_ps(&Ax[i]);
