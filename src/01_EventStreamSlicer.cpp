...
int main() {
    ...
    slicer.doEveryTimeInterval(1000ms, [](const dv::EventStore &events) {
        std::cout << "* Received events time-based slicing: " << events << std::endl;
    });

    // Register this method to be called every 100 events
    slicer.doEveryNumberOfElements(10, [](const dv::EventStore &events) {
    ...
    const dv::EventStore store = dv::data::generate::uniformEventsWithinTimeRange(0, 10s, cv::Size(100, 100), 1000);
